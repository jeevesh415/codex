...
- `plugin/list` — list discovered marketplaces reachable from optional `cwds` (unioned into a single list). When `cwds` is omitted, only home-scoped marketplaces are considered. Includes each plugin's current `enabled` state from config and optional `plugin.json` `interface` metadata (asset paths are returned as absolute paths) (**under development; do not call from production clients yet**).
